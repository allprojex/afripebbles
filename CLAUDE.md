...
- `artifacts/api-server` binds to `HOST` (default `127.0.0.1`, loopback-only) and `PORT` — see `src/lib/serverConfig.ts`. Production sets `HOST=127.0.0.1` explicitly so the API is never reachable except through Nginx; only override to `0.0.0.0` if something genuinely needs to reach the process from outside the host.
